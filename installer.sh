...
function waFindInstalled() {
	...
	if [ $USEDEMO != 1 ]; then
		...
		xfreerdp /d:"${RDP_DOMAIN}" /u:"${RDP_USER}" /p:"${RDP_PASS}" /v:${RDP_IP} +auto-reconnect +home-drive -wallpaper /span /wm-class:"RDPInstaller" /app:icon:"${DIR}/../icons/windows.svg",program:cmd.exe,cmd:"/C \\\\tsclient\\home\\.local\\share\\winapps\\installed.bat" 1> /dev/null 2>&1 &
		COUNT=0
